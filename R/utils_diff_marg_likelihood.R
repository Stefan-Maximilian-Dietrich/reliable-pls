...
get_log_marg_l <- function(logistic_model) {
  ...
  try({
      ...
      not_nas <- vcov(logistic_model)[!is.na(vcov(logistic_model))] ## 0 Falls NA's, ist ein Vektor 
      v_cov_matrix <- matrix(not_nas, ncol = sqrt(length(not_nas))) ## ist wieder Matrix 
      fisher_info <- v_cov_matrix %>% solve() ## Matrix
      log_marg_l <- logLik(logistic_model) + n_parameters/2 * log(2*pi/n_obs) - 0.25 * log(det(fisher_info)) #von 3.2
      }else{
