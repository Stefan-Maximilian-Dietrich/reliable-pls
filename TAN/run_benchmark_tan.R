for(j in 1:length(Experiments)){
  ...
  prioris <- as.data.table(gerate_priori_simplex_rec(levels_present, refinement))[,prioID:= 1:.N ] # muss neu gemacht werden 
  ...
  with_progress({
    ...
    gamma_l <- future_map(1:N, function(i) {
        ...
        dt_train <- as.data.table(discretize_rec_min_ent(copy(data_samp[[1]])))
        dt_test <- as.data.table(discretize_rec_min_ent_test(copy(data_samp[[1]]), copy(data_samp[[3]])))
        dt_unlabled <- as.data.table(discretize_rec_min_ent_test(copy(data_samp[[1]]), copy(data_samp[[2]])))
        ...
        if(structure == "NB") {
          if(methods$e_admissible){
            e_admissible_g <- nb_SSL_E_admissible(prioris = prioris, dt_train = dt_train, dt_unlabled = dt_unlabled, dt_test = dt_test, alpha = alpha)

          } else {e_admissible_g <- NA}
          
          if(methods$maximal){
            maximal_g <-nb_SSL_maximal(prioris = prioris, train = dt_train, unlabeld = dt_unlabled, test = dt_test, alpha = alpha)
            
          } else {maximal_g <- NA}
          
          if(methods$SSL){
            SSL_g<-nb_SSL(prioris = prioris, dt_train = dt_train, dt_unlabled = dt_unlabled, dt_test = dt_test, alpha = alpha)
            
          } else {SSL_g <- NA}
          
          if(methods$SL){
            SL_g <- nb_SL(prioris = prioris, dt_train = dt_train, dt_unlabled = dt_unlabled, dt_test = dt_test, alpha = alpha)
      ...
      l <- list(e_admissible=e_admissible_g,SSL = SSL_g, SL= SL_g, SSL_variance = SSL_variance,SSL_entropy = SSL_entropy,  maximal = maximal_g, M_MaxiMin= M_MaxiMin, M_MaxiMax = M_MaxiMax)
